...
interface KitchenAssistantTableProps {
  candidates: Candidate[];
  scores: Record<string, Record<string, number>>;
  onViewScores: (candidate: Candidate) => void;
}

export function KitchenAssistantTable({ candidates, scores, onViewScores }: KitchenAssistantTableProps) {
  return (
      ...
      <table className="w-full text-sm">
        <thead className="bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            <th className="px-4 sm:px-6 py-2 text-left font-semibold text-gray-700">{SCORE_AREA_LABELS.COMPANIONSHIP}</th>
            <th className="px-4 sm:px-6 py-2 text-left font-semibold text-gray-700">{SCORE_AREA_LABELS.PATIENCE}</th>
            <th className="px-4 sm:px-6 py-2 text-left font-semibold text-gray-700">{SCORE_AREA_LABELS.MEDICATION_MANAGEMENT}</th>
            <th className="px-4 sm:px-6 py-2 text-left font-semibold text-gray-700">{SCORE_AREA_LABELS.FIRST_AID}</th>
            <th className="px-4 sm:px-6 py-2 text-left font-semibold text-gray-700">{SCORE_AREA_LABELS.ELDERLY_CARE}</th>
          ...
          {candidates.map((candidate) => {
            ...
            return (
              <tr key={candidate.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 sm:px-6 py-3 text-gray-600">{candidateScore.COMPANIONSHIP || '-'}</td>
                <td className="px-4 sm:px-6 py-3 text-gray-600">{candidateScore.PATIENCE || '-'}</td>
                <td className="px-4 sm:px-6 py-3 text-gray-600">{candidateScore.MEDICATION_MANAGEMENT || '-'}</td>
                <td className="px-4 sm:px-6 py-3 text-gray-600">{candidateScore.FIRST_AID || '-'}</td>
                <td className="px-4 sm:px-6 py-3 text-gray-600">{candidateScore.ELDERLY_CARE || '-'}</td>
