...
interface HousekeeperTableProps {
  candidates: Candidate[];
  scores: Record<string, Record<string, number>>;
  onViewScores: (candidate: Candidate) => void;
}

export function HousekeeperTable({ candidates, scores, onViewScores }: HousekeeperTableProps) {
  return (
      ...
      <table className="w-full text-sm">
        <thead className="bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            <th className="px-4 sm:px-6 py-2 text-left font-semibold text-gray-700">{SCORE_AREA_LABELS.CLEANING}</th>
          ...
          {candidates.map((candidate) => {
            ...
            return (
              <tr key={candidate.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 sm:px-6 py-3 text-gray-600">{candidateScore.CLEANING || '-'}</td>
