...
function EvaluationManagement() {
  ...
  const handleOpenCompleteModal = async (item: EvaluationListItem) => {
    try {
      const fullEvaluation = await evaluationService.getEvaluation(item.id);
      setSelectedEvaluation(fullEvaluation);
  ...
  const handleCreateEvaluation = async (data: CreateEvaluationData): Promise<boolean> => {
    ...
    try {
      ...
      setSubmitError(
        error?.detail || error?.error || error?.response?.data?.detail || t('scheduleFailed')
      );
      return false;
    }
  };
  ...
  const handleEditEvaluation = async (data: CreateEvaluationData): Promise<boolean> => {
    ...
    try {
      ...
      setSubmitError(
        error?.detail || error?.error || error?.response?.data?.detail || t('updateFailed')
      );
      return false;
    }
  };
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
              <button
                ...
                className="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-medium text-sm transition-colors flex items-center justify-center gap-2"
              >
                {t("startAiInterview")}
              </button>
              <button
                onClick={handleOpenCreateModal}
                className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg font-medium text-sm transition-colors flex items-center justify-center gap-2"
              >
