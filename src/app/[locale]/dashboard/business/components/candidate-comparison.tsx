...
// Dashboard-home leaderboard, not the full interactive radar comparison on
// the Candidates page - calling getCandidateComparison() with no ids hits
// the same backend endpoint's other mode (B2BCandidateComparisonView),
// which auto-returns the top-scored candidates instead of requiring a
// selection. Mirrors indivisual/components/candidate-comparison.tsx.
export function CandidateComparison() {
  const t = useTranslations("dashboard.business.candidateComparison")
  ...
  const fetchData = async () => {
    ...
    setError(null)
    try {
      const comparisonData = await b2bDashboardService.getCandidateComparison()
