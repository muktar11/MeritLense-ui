...
// different lighting. Real candidates with genuine matches were still
// failing at a 40% bar, so this sits closer to the permissive edge of the
// same-person range (distance <= 0.48 of the 0.6 same-person boundary) to
// prioritize not blocking real matches.
// Keep this aligned with IDENTITY_VERIFICATION_MATCH_THRESHOLD on the
// backend (settings + InterviewSession.candidate_prechecks_complete) so the
// UI never reports success for a session that cannot start.
